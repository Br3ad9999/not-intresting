...
const sessionCookieName = "ironforge.sid";
const sessionSecret =
  process.env.SESSION_SECRET ||
  (isProduction ? "" : "ironforge-dev-session-secret");
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!fullName || !email || !password || !requestedPlanId) {
      return res
        .status(400)
        .json({ error: "Full name, email, password, and plan are required." });
    }
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!profile) {
      ...
      return res
        .status(401)
        .json({ error: "Your session is no longer valid." });
    }
...
app.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const membershipStatus = String(req.body.membershipStatus || "")
      .trim()
      .toLowerCase();
    ...
    if (!memberId || !planId || !membershipStatus) {
      return res
        .status(400)
        .json({ error: "Member, plan, and membership status are required." });
    }

    if (
      !["active", "expired", "cancelled", "pending"].includes(membershipStatus)
    ) {
      return res
        .status(400)
        .json({ error: "Invalid membership status supplied." });
    }
    ...
    await withTransaction(async (client) => {
      ...
      await client.query(
        "UPDATE members SET is_active = $1 WHERE member_id = $2",
        [isActive, memberId],
      );
